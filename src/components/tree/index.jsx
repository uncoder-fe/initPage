...
class Tree extends React.Component {
	...
	onSelect(nodes, item, args) {
		// 隐藏同级
		nodes.forEach((ite) => { ite.visibility = false })
		// 子元素隐藏
	...
	loop(nodes, level = 0) {
		...
		return nodes.map((item) => {
			...
			if (sons && sons.length > 0) {
				...
				return <TreeNode key={id} highlight={highlight} level={level} visibility={visibility || false} title={name} nodes={childrenNodes} onSelect={() => this.onSelect(nodes, item, visibility || false)} />
			}
			// console.log("子集::=>", name);
			return <TreeNode key={id} highlight={highlight} level={level} title={name} onSelect={() => this.onSelect(nodes, item, visibility || false)} />
		})
	}
	storeResult(item) {
		const { id, level } = item
